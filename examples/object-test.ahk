class MyObjectTest extends AhkUnit {
	...
	myTest1() {
		...
		d.it("Width should be 100").expect(this.myObj.width).toBe(100)
		d.it("Height should be 200").expect(this.myObj.height).toBe(200)
	}
	...
	myTest2() {
		...
		;toBeTrue()  is x true
		x := false
		d.it("x should be true").expect(x).toBeTrue()

		;toBeFalse() is x false
		x := true
		d.it("x should be false").expect(x).toBeFalse()
		...
		x := 100
		d.it("x should be greater than 101").expect(x).toBeGreaterThan(101)
